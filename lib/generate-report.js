...
INCLUDE WHAT HAPPENED: Use the guide's specific situations and examples—this is how we tell the partner. Include group size, concrete behaviors (e.g. interrupting, arguing among themselves, not respecting timing), and specific incidents (e.g. someone going ahead alone, someone upset, always late). Describe what actually happened in tactful, professional language. Do not replace details with vague phrases like "challenging behavior"; keep the substance.

LENGTH: Keep paragraphs SHORT—2 to 4 sentences max. Be concise. No lengthy prose or wordy paragraphs. Get to the point quickly.

TONE: ${toneConfig.instruction}

PLATFORM: ${platformConfig.guidelines}

STRUCTURE: Greeting → brief context (booking/guest) → what actually happened (specifics from the guide) → how our guide handled it → proactive note / commitment to quality → closing and signature.`;
...
Output plain text only, ready to paste into an email. No markdown. Use short paragraphs (2–4 sentences each). Keep it concise. Include the specific situations from the guide's feedback (e.g. group size, interrupting, arguing, particular incidents like elevator / left behind / crying, timing issues) so the partner knows what happened—write how we tell them.`;
